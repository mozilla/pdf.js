...
const fillDependencies = ["fillStyle", "globalAlpha"];
...
- imageSmoothingQuality (not used)
- shadowBlur (not used)
- shadowColor (not used)
- shadowOffsetX (not used)
- shadowOffsetY (not used)
*/
...
class CanvasRecorder {
  ...
  #registerTransformDependency() {
    const dependenciesSet = this.#currentGroup.dependencies;
    this.#transformIds.forEach(dependenciesSet.add, dependenciesSet);
  }
  ...
  fillText(text, x, y, maxWidth) {
    ...
    this.#registerTransformDependency();
  ...
  save() {
    ...
    this.#transformIds = Object.create(this.#transformIds);
    this.#ctx.save();
  }
  ...
  restore() {
    ...
    const prevTransforms = Object.getPrototypeOf(this.#transformIds);
    if (prevTransforms !== Array.prototype) {
      this.#transformIds = prevTransforms;
    }
    this.#ctx.restore();
  }

  setTransform(a, b, c, d, e, f) {
    this.#transformIds.length = 0;
    if (this.#nextCommandsId !== -1) {
      this.#transformIds.push(this.#nextCommandsId);
    }
  ...
  static {
    ...
    if (typeof CanvasRenderingContext2D !== "undefined") {
      ...
      const transformMethods = ["transform", "translate", "rotate", "scale"];
      for (const name of transformMethods) {
