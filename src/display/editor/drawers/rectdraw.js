...
class RectDrawOutliner {
  ...
  add(x, y) {
    ...
    const newLines = [...this.#first, ...this.#last];
    this.#line.splice(0, this.#line.length, ...newLines);
  ...
  toSVGPath() {
    const firstX = Outline.svgRound(this.#first[4]);
    const firstY = Outline.svgRound(this.#first[5]);
    const lastX = Outline.svgRound(this.#last[4]);
    const lastY = Outline.svgRound(this.#last[5]);

    const [x1, y1] = [firstX, firstY];
    const [x2, y2] = [lastX, lastY];

    // Calculate the other two corners
    const x3 = x2,
      y3 = y1; // Top-right corner
    const x4 = x1,
      y4 = y2; // Bottom-left corner

    this.#lastSVGPath = `M ${x1} ${y1} L ${x3} ${y3} L ${x2} ${y2} L ${x4} ${y4} Z`;
...
class RectDrawOutline extends Outline {
  ...
  toSVGPath() {
    const points = this.#lines[0].points;

    const firstX = Outline.svgRound(points[0]);
    const firstY = Outline.svgRound(points[1]);
    const lastX = Outline.svgRound(points[2]);
    const lastY = Outline.svgRound(points[3]);
    ...
    return `M ${x1} ${y1} L ${x3} ${y3} L ${x2} ${y2} L ${x4} ${y4} Z`;
  }
