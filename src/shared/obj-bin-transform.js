...
class SubstitionInfo {
  ...
  static write(substitionInfo) {
    ...
    stringsLength += 4;
    const encodedStyleStyle = encoder.encode(substitionInfo.style.style);
    const encodedStyleWeight = encoder.encode(substitionInfo.style.weight);
    const styleLength =
      4 + encodedStyleStyle.length + 4 + encodedStyleWeight.length;
    const lengthEstimate = 1 + stringsLength + styleLength;
