...
import { DrawOPS, info, OPS, shadow } from "../shared/util.js";
...
class CompiledType3Glyph {
  static MAX_SIZE = 1000; // To disable Type3 compilation, set the value to `-1`.
  ...
  constructor({ data: img, width, height }) {
    const { MAX_SIZE, MAX_POINTS, POINT_TYPES } = CompiledType3Glyph;
    ...
    const pathBuf = this.#pathBuffer;
    this.#pathMinMax = [0, 0, width, height];
  ...
  // Always copy the data, since the compiled Type3-glyph may be cached.
  get pathArgs() {
    if (!this.#pathMinMax) {
      return null;
    }
    return [
      OPS.fill,
      [new Float32Array(this.#pathBuffer)],
      new Float32Array(this.#pathMinMax),
    ];
  }
}
