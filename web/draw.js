  ...
  fabricLoadTemplate: function(json) {
    ...
    for (var i = 1; i <= PDFViewerApplication.pdfViewer.pagesCount; i++) {
      ...
      renderFinishedPromise.then(function(pageNum){
        var canvas = self.getCanvas(pageNum);
        var obj = json[pageNum - 1],
            oldBg = canvas.backgroundImage._originalElement,
            container = document.querySelector('.page[data-page-number="' + pageNum + '"] .canvasWrapper');
        console.log(obj);
