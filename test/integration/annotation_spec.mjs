...
describe("ResetForm action", () => {
  ...
  describe("Text under some annotations", () => {
    describe("bug1885505.pdf", () => {
      ...
      it("must check that an highlight as a mark element", async () => {
        await Promise.all(
          pages.map(async ([browserName, page]) => {
      ...
      it("must check that an underline as a u element", async () => {
        await Promise.all(
          pages.map(async ([browserName, page]) => {
            const text = await page.$eval(
      ...
      it("must check that an highlight as a squiggly/u element", async () => {
        await Promise.all(
          pages.map(async ([browserName, page]) => {
            const text = await page.$eval(
...
a dynamic compiler for JavaScript based on our`);
      ...
      it("must check that an highlight as a s element", async () => {
        await Promise.all(
          pages.map(async ([browserName, page]) => {
            const text = await page.$eval(
